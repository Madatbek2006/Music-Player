...
class MyService : Service() {
    ...
    private val receiver = com.example.mymusicservise.presenter.service.MyBroadcast()
    ...
    private fun doneCommand(command: ServiceEnum) {
        val data: MusicData =
            if (MyMusicModel.isFavorite) MyMusicModel.favoriteList!![MyMusicModel.pos2]
            else MyMusicModel.allList!![MyMusicModel.pos1]
        when (command) {
            ServiceEnum.PLAY -> {
                MyMusicModel.fullTime = data.duration
                MyMusicModel.id = data.id

                " ServiceEnum.PLAY".myLog()
                MyMusicModel.isPlayingLiveData.value = true
                if (MyMusicModel.isFavorite){
                    if (oldPos != MyMusicModel.pos2) {
...
//                mediaPlayer.seekTo(MyMusicModel.currentTime.toInt())
                ...
                startMyService(true)
                if (MyMusicModel.isFavorite){
                    oldPos = MyMusicModel.pos2

                }else{
                    oldPos = MyMusicModel.pos1
            ...
            ServiceEnum.NEXT -> {
                ...
                if (myShar.isShuffle()) {
                    if (MyMusicModel.isFavorite){
                        MyMusicModel.pos2 = ranDomNumber(MyMusicModel.pos2)
                    }else{
                        MyMusicModel.pos1 = ranDomNumber(MyMusicModel.pos1)
                    }
                } else {
                    if (!MyMusicModel.isFavorite) {
                        if (MyMusicModel.pos1 + 1 == MyMusicModel.allList!!.size) MyMusicModel.pos1 =
                            0
                        else MyMusicModel.pos1++
                    } else {
                        if (MyMusicModel.pos2 + 1 == MyMusicModel.favoriteList!!.size) MyMusicModel.pos2 =
                            0
                        else MyMusicModel.pos2++
                    }
                }
            ...
            ServiceEnum.PREV -> {
                ...
                if (myShar.isShuffle()) {
                    ...
                    if (MyMusicModel.isFavorite) {
                        if (MyMusicModel.pos2 == 0) MyMusicModel.pos2 =
                            MyMusicModel.favoriteList!!.size - 1
                        else MyMusicModel.pos2--
                    } else {
                        if (MyMusicModel.pos1 == 0) MyMusicModel.pos1 =
                            MyMusicModel.allList!!.size - 1
                        else MyMusicModel.pos1--
                    }
                }
                doneCommand(ServiceEnum.PLAY)
    ...
    private fun startMyService(isPlaying: Boolean) {
        ...
        val musicData =
            if (MyMusicModel.isFavorite) MyMusicModel.favoriteList!![MyMusicModel.pos2] else MyMusicModel.allList!![MyMusicModel.pos1]
        val bitmap: Bitmap
